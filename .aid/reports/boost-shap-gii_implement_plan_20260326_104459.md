<implement_plan>
 ...
 <changes>
 <change id="change-1" priority="P0" source_item="decision-3, decision-6">
 ...
 <change id="change-2" priority="P0" source_item="decision-1, decision-2">
 ...
 <risk>medium - must be followed immediately by (import rewrite) or imports break</risk>
 <rollback>git mv each file back to root</rollback>
 </change>

 <change id="change-3" priority="P0" source_item="decision-10, decision-11">
 ...
 <change id="change-4" priority="P0" source_item="decision-9">
 ...
 <change id="change-5" priority="P0" source_item="decision-7">
 ...
 <change id="change-6" priority="P0" source_item="decision-1, decision-2, decision-4, decision-5, decision-6">
 ...
 <change id="change-7" priority="P1" source_item="decision-3">
 ...
 <change id="change-8" priority="P1" source_item="decision-5, check_env.py audit">
 ...
 <dependencies> (file must be in new location)</dependencies>
 <risk>low - removing a check for an unused dependency</risk>
 <rollback>re-add "shap" to PYTHON_DEPS</rollback>
 </change>

 <change id="change-9" priority="P1" source_item="gitignore update for new layout">
 ...
 <dependencies> (files must be in new locations)</dependencies>
 ...
 <change id="change-10" priority="P1" source_item="decision-6, environment.yaml sync">
 ...
 <change id="change-11" priority="P2" source_item="module runnability">
 <file path="src/boost_shap_gii/train.py" action="modify" />
 <file path="src/boost_shap_gii/predict.py" action="modify" />
 <file path="src/boost_shap_gii/infer.py" action="modify" />
 <file path="src/boost_shap_gii/check_env.py" action="modify" />
 <description>Ensure each module remains directly runnable via `python -m boost_shap_gii.train` by keeping the `if __name__ == "__main__": main` guard. No changes needed if already present (they are), but confirm the guard is intact after refactoring.</description>
 <spec>Verification pass only. Each file already has `if __name__ == "__main__": main`. Ensure did not accidentally remove them.</spec>
 <dependencies></dependencies>
 ...
 1. **** (LICENSE) and **** (environment.yaml) - Independent, no dependencies. Can be done first/in parallel.
 2. **** (__init__.py) and **** (pyproject.toml) - Create package skeleton. requires the directory to exist.
 3. **** (git mv files) - Move all Python source into package directory.
 4. **** (relative imports) - Must immediately follow to restore import correctness.
 5. **** (remove shap from check_env) - Small modification after file is in place.
 6. **** (move scripts) - Move plot.R and shell script, update shell script paths.
 7. **** (cli.py) - Create CLI entry point. Depends on all modules being in their final locations.
 8. **** (.gitignore) - Update after all files are in their final locations.
 9. **** (verification) - Final check that __main__ guards survived.
