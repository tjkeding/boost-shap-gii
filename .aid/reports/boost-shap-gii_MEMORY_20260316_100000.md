<session_memory>
 ...
 <work_completed>
 <item>Completed brainstorm triage of all 25 CR findings (findings 10-25; findings 1-9 done in Session 2). Final tally: 13 code fixes, 2 doc items, 12 no-action (including 2 reversed from doc-only to no-action by user).</item>
 ...
 <decisions>
 <decision context="Finding 10: Bootstrap CI failure handling" choice="Drop-and-warn with n_boot_effective reporting" rationale="Bootstrap failures (single-class resamples) are diagnostic of class imbalance, not retryable. n_boot_effective serves as confidence proxy." />
 <decision context="Finding 14: SHAP interaction matrix convention" choice="No code change — current summed convention is correct" rationale="GII is prediction decomposition. Off-diagonal SHAP cells contain phi(i,j)/2; full matrix sum recovers true Shapley interaction index. Using one triangle would UNDERSTATE interactions by 2x." />
 <decision context="Finding 15: 2D spline fallback" choice="Three-way routing: both-fail→group_means_2d, one-fail→stacked_spline" rationale="Original 0.0 return was incorrect; stacked-spline inherits full 1D energy gate protection. Group-means only when both axes lack resolution." />
 <decision context="Finding 16: NaN preservation in bootstrap" choice="Remove NaN-to-zero replacement; preserve NaN" rationale="NaN distinguishes spline failure from genuinely zero V. Downstream nanmean/nanpercentile handle NaN natively." />
 <decision context="Finding 23: Permutation test robustness" choice="While-loop with 2x n_perm attempt cap" rationale="Permutation failures are rare numerical artifacts (not diagnostic). Retry is valid because permutations preserve full y distribution." />
 <decision context="Finding 25: Plot left panel" choice="Changed from GII distribution to M distribution" rationale="Two-panel decomposition: M = magnitude (left), V = variability pattern (right). X-axis label: 'Importance Magnitude (M)'." />
 <decision context="Commit message style" choice="Descriptive, no internal labels" rationale="User feedback: CR finding labels (all 25 critical-review findings) are internal and should not appear in commits or external content." />
 </decisions>
 <known_issues>
