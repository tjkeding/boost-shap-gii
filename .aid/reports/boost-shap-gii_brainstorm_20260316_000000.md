<brainstorm_report>
 ...
 <file path="train.py" relevance=" (inner CV seed), (ordinal validation), (shadow model iterations)" />
 <file path="predict.py" relevance=" (syntax error), (model count validation)" />
 <file path="shap_utils.py" relevance="findings 14-25 (statistical/algorithmic findings)" />
 <file path="utils.py" relevance=" (config defaults), (bootstrap CI), (permutation test)" />
 <file path="plot.R" relevance=" (N_CORES cap), (left panel M vs GII)" />
 <file path="check_env.py" relevance=" (missing deps)" />
 </context_files>

 <topics>

 <!-- ================================================================= -->
 <!-- SESSION 1: findings 1-9 (decided 2026-03-09/10) -->
 <!-- ================================================================= -->

 <topic id="topic" title="Finding 1: Syntax error in predict.py:4">
 ...
 <topic id="topic" title="Finding 2: Inner CV seed reuse">
 ...
 <topic id="topic" title="Finding 3: Ordinal unknown-value threshold metric">
 <summary>Uses fraction of unique values, not fraction of observations. Can miss high-frequency unknowns.</summary>
 <approaches>
 <approach label="Two-tier check" feasibility="high" risk="low">
 ...
 <decision status="decided" chosen="">Two-tier approach. Hardcoded thresholds are fine — these are safety guards, not tunable parameters.</decision>
 </topic>

 <topic id="topic" title="Finding 4: Shadow model iteration count">
 ...
 <approaches>
 <approach label="Early stopping with outer validation fold" feasibility="high" risk="low">
 ...
 <decision status="decided" chosen="">Early stopping with outer validation fold, tuned_iters * 2 ceiling, same early_stopping_rounds patience.</decision>
 </topic>

 <topic id="topic" title="Finding 5: predict.py model count validation">
 ...
 <topic id="topic" title="Finding 6: Depth search space too restrictive for small n">
 ...
 <topic id="topic" title="Finding 7: one_hot_max_size bound uses feature count">
 ...
 <topic id="topic" title="Finding 8: check_env.py missing joblib and statsmodels">
 ...
 <topic id="topic" title="Finding 9: __NA__ sentinel undocumented">
 ...
 <!-- ================================================================= -->
 <!-- SESSION 2: findings 10-25 (decided 2026-03-16) -->
 <!-- ================================================================= -->

 <topic id="topic" title="Finding 10: Bootstrap CI drops resamples with &lt;2 unique y values">
 ...
 <approaches>
 <approach label="Drop and warn with n_boot_effective" feasibility="high" risk="low">
 ...
 <decision status="decided" chosen="">Drop-and-warn with n_boot_effective reporting. n_boot_effective serves as a transparent confidence proxy (n_boot_effective = n_boot indicates maximum confidence).</decision>
 </topic>

 <topic id="topic" title="Finding 11: plot.R N_CORES cap">
 ...
 <topic id="topic" title="Finding 12: Column ordering (note)">
 ...
 <topic id="topic" title="Finding 13: Ensemble soft voting calibration (note)">
 ...
 <topic id="topic" title="Finding 14: Interaction SHAP 2× scale">
 ...
 <approaches>
 <approach label="Keep summed convention (current code)" feasibility="high" risk="low">
 ...
 <decision status="decided" chosen="">No code change. Current summed convention is correct for GII's prediction-decomposition interpretation. Document that SHAP interaction matrices divide phi(i,j) by 2 per cell, so the full matrix is required to recover the true interaction contribution.</decision>
 </topic>

 <topic id="topic" title="Finding 15: 2D spline zero-return fallback">
 ...
 <approaches>
 <approach label="Stacked-spline routing" feasibility="high" risk="low">
 ...
 <decision status="decided" chosen="">Replace return 0.0 with stacked-spline routing based on which axis failed knot placement. Group-means only when both axes lack resolution.</decision>
 </topic>

 <topic id="topic" title="Finding 16: NaN-to-zero bootstrap bias">
 ...
 <topic id="topic" title="Finding 17: Boruta adaptive noise baseline">
 ...
 <topic id="topic" title="Finding 18: Missing +1 correction on exceedance p-values">
 ...
 <topic id="topic" title="Finding 19: 2D energy check heuristic limitation">
 <summary>Axis-wise total variation is a heuristic that may miss oblique oscillation. However, the stability gate provides downstream protection, and the reduces the scope of 2D spline usage.</summary>
 <decision status="decided" chosen="no_action">No action. No documentation needed.</decision>
 </topic>

 <topic id="topic" title="Finding 20: Bootstrap method-switching at discrete threshold">
 ...
 <topic id="topic" title="Finding 21: Stability metric for zero-variance estimates">
 ...
 <topic id="topic" title="Finding 22: NaN mask alignment fragility">
 ...
 <topic id="topic" title="Finding 23: Permutation test effective count reporting">
 ...
 <approaches>
 <approach label="While-loop with cap" feasibility="high" risk="low">
 ...
 <decision status="decided" chosen="">While-loop with 2 * n_perm cap. Warn and proceed with effective count if cap reached.</decision>
 </topic>

 <topic id="topic" title="Finding 24: SHAP decomposition non-additivity (note)">
 <summary>GII values cannot be summed to reconstruct marginal SHAP importance. Inherent property of the framework.</summary>
 <decision status="decided" chosen="no_action">No action. No documentation needed — same reasoning as (do not document hypothetical misuse).</decision>
 </topic>

 <topic id="topic" title="Finding 25: GII insensitivity to constant-effect features (note) + Plot panel change">
 ...
 <approaches>
 <approach label="Change left plot panel from GII to M" feasibility="high" risk="low">
 ...
 <decision status="decided" chosen="">Change left panel to M distribution. X-axis label: "Importance Magnitude (M)". No other plotting changes. Data files already exist (bootstrap_distributions_M.parquet, stratified_noise_distributions_M.parquet).</decision>
 </topic>
 ...
 <action_items>
 <!-- P0 -->
 <item priority="P0" target_mode="implement" description="Finding 1: Remove trailing ß from predict.py:4" />

 <!-- P1 — Code fixes -->
 <item priority="P1" target_mode="implement" description="Finding 2: Offset inner CV seed by fold_idx + 1 in train.py" />
 <item priority="P1" target_mode="implement" description="Finding 3: Add two-tier ordinal unknown check (unique-value + observation-level fraction) in train.py, predict.py, infer.py" />
 <item priority="P1" target_mode="implement" description="Finding 4: Shadow model early stopping with outer val fold, 2x iteration ceiling, same early_stopping_rounds patience in train.py" />
 <item priority="P1" target_mode="implement" description="Finding 5: Add model-count vs fold-count assertion in predict.py" />
 <item priority="P1" target_mode="implement" description="Finding 15: Replace return 0.0 in calculate_v_spline_2d with stacked-spline routing (low-resolution axis as grouping var); group-means only when both axes lack resolution" />
 <item priority="P1" target_mode="implement" description="Finding 16: Remove NaN-to-zero replacement (shap_utils.py lines 709-711); let existing nanmean/nanpercentile handle exclusion natively" />
 <item priority="P1" target_mode="implement" description="Finding 18: Apply +1 correction to all exceedance p-values (M, V, GII) in shap_utils.py: p = (sum + 1) (n_boot + 1)" />

 <!-- P2 — Code fixes -->
 <item priority="P2" target_mode="implement" description="Finding 6: Add floor to depth search space: max(3...) in utils.py" />
 <item priority="P2" target_mode="implement" description="Finding 7: Fix one_hot_max_size to fixed [2, 25] range in utils.py" />
 <item priority="P2" target_mode="implement" description="Finding 8: Add joblib and statsmodels to PYTHON_DEPS in check_env.py" />
 <item priority="P2" target_mode="implement" description="Finding 10: Add n_boot_effective tracking and >5% drop warning to compute_bootstrap_ci in utils.py" />
 <item priority="P2" target_mode="implement" description="Finding 11: Cap N_CORES at min(config, detectCores) with NA fallback in plot.R" />
 <item priority="P2" target_mode="implement" description="Finding 23: Convert permutation test to while-loop with 2*n_perm cap, warn if cap reached, in utils.py" />
 <item priority="P2" target_mode="implement" description="Finding 25: Change left plot panel from GII to M distribution; x-axis label 'Importance Magnitude (M)'; update get_global_x_limit_dir to use M parquets in plot.R" />

 <!-- P2 — Documentation -->
 <item priority="P2" target_mode="document" description="Finding 9: Document __NA__ sentinel behavior for nominal missingness in README and config comments" />
 <item priority="P2" target_mode="document" description="Finding 14: Document that SHAP interaction matrices divide phi(i,j) by 2 per cell; full matrix required to recover true interaction contribution" />
